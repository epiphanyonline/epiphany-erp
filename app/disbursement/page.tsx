...
export default function LoanDisbursementPage() {
  const { staff, loading: staffLoading, canDisburse } = useCurrentStaff()
  ...
  const [result, setResult] = useState<LoanDisbursementResult | null>(null)
  ...
  }, [selectedMember, principalAmount, tenureDays, staff, businessDate, canDisburse])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const numericTenure = Number(tenureDays)

    if (!staff) {
      setErrorText('Your session has expired. Please login again.')
      setLoading(false)
      return
    }

    if (!canDisburse) {
      setErrorText('Only ADMIN or SUPERVISOR can disburse loans.')
      setLoading(false)
      return
    }
    ...
    if (!numericPrincipal || numericPrincipal <= 0) {
      setErrorText('Principal amount must be greater than zero.')
      setLoading(false)
      return
    }

    if (!numericTenure || numericTenure <= 0) {
    ...
    const { data, error } = await supabase.rpc('post_loan_disbursement', {
      ...
      p_principal_amount: numericPrincipal,
      p_tenure_days: numericTenure,
    ...
    console.log('disbursement error:', error)
    ...
    if (data && data.length > 0) {
      setResult(data[0] as LoanDisbursementResult)
    } else {
      setErrorText('No response returned from loan disbursement function.')
    }
    ...
    return (
      <main style={styles.page}>
        <div style={styles.pageInner}>
          <p style={styles.subtitle}>Loading access...</p>
        </div>
      </main>
    )
  }
        ...
        <div style={styles.pageInner}>
          <section style={styles.card}>
            ...
            <p style={styles.subtitle}>
              Only ADMIN or SUPERVISOR can access loan disbursement.
            </p>
          </section>
        </div>
      </main>
    ...
    <main style={styles.page}>
      <div style={styles.pageInner}>
        <div style={styles.headerRow}>
          <div>
            <h1 style={styles.title}>Loan Disbursement</h1>
            <p style={styles.subtitle}>Create a new loan for an eligible member</p>
          </div>
            ...
            <div style={styles.formGrid}>
              ...
              <div style={styles.field}>
                ...
                <p style={styles.helperText}>
                  This is taken from the logged-in session and cannot be changed.
                </p>
              </div>
              ...
              <div style={styles.field}>
                ...
                <input
                  ...
                  placeholder="Approval / voucher / batch reference"
                />
              </div>
          ...
          {result ? (
            ...
            >
              <h3 style={{ marginTop: 0 }}>
                {result.success ? 'Loan Disbursed' : 'Loan Disbursement Failed'}
              </h3>

              <p><strong>Message:</strong> {result.message}</p>
              <p><strong>Member:</strong> {result.member_name || '-'} ({result.member_code || '-'})</p>
              <p><strong>Principal:</strong> ₦{Number(result.principal_amount || 0).toLocaleString()}</p>
              <p><strong>Outstanding Balance:</strong> ₦{Number(result.outstanding_balance || 0).toLocaleString()}</p>
              <p><strong>Expected Daily:</strong> ₦{Number(result.expected_daily_amount || 0).toLocaleString()}</p>
              <p><strong>Due Date:</strong> {result.due_date || '-'}</p>
              <p><strong>Transaction Ref:</strong> {result.tx_ref || '-'}</p>
              <p><strong>Loan Status:</strong> {result.loan_status || '-'}</p>
            </div>
          ) : null}
        </section>
      </div>
