...
export default function SavingsDepositPage() {
  ...
  const [result, setResult] = useState<SavingsDepositResult | null>(null)
  ...
  }, [selectedMember, amount, staff, businessDate, accountType])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!canSeeAll && staffScope?.park_id && selectedMember.main_park_id !== staffScope.park_id) {
      setErrorText('You can only post savings deposit for members in your assigned park.')
      setLoading(false)
      return
    }

    const { data, error } = await supabase.rpc('post_savings_deposit', {
    ...
    if (data && data.length > 0) {
      setResult(data[0] as SavingsDepositResult)
    } else {
      setErrorText('No response returned from savings deposit function.')
    }
    ...
    <main style={styles.page}>
      <div style={styles.pageInner}>
        <div style={styles.headerRow}>
          <div>
            <h1 style={styles.title}>Post Savings Deposit</h1>
            <p style={styles.subtitle}>Record regular or compulsory savings deposit</p>
          </div>
          ...
          {result ? (
            ...
            >
              <h3 style={{ marginTop: 0 }}>
                {result.success ? 'Savings Deposit Posted' : 'Savings Deposit Failed'}
              </h3>
